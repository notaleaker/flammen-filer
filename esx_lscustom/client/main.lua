		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				if (data.current.label == " ".._U('by_default') and GetVehicleMod(vehicle, v.modType) ~= -1) or string.match(data.current.label, _U('installed')) then
					...
					if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'lsc' or ESX.PlayerData.job.name == 'exotic' then
...
function GetAction(data)
	...
	if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'mechanic' or ESX.PlayerData.job.name == 'exotic' then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local lastZone    = nil
			local vehicle = GetVehiclePedIsIn(playerPed, false)
			if (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'mechanic' or ESX.PlayerData.job.name == 'exotic') or Config.IsMechanicJobOnly == false then
				for k,v in pairs(Config.Zones) do
			...
			if IsControlJustReleased(0, Keys['E']) and not lsMenuIsShowed and isInLSMarker then
				if not IsSpecial or (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'mechanic' or ESX.PlayerData.job.name == 'exotic') then
