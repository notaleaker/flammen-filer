...
Citizen.CreateThread(function()
    ...
    while true do
        if FinanceOrganization.Office.needToLoad then
            -- Need to load
            if (Global.FinanceOffices.isInsideOffice2) then
                if LastOfficeID ~= 2 then
                    FinanceOrganization.Office.Clear()
                end
                DrawOrganizationName('Lux', FinanceOrganization.Name.style, FinanceOrganization.Name.Colors.black, FinanceOrganization.Name.font)
                LastOfficeID = 2
            ...
            elseif (Global.FinanceOffices.isInsideOffice3) then
                if LastOfficeID ~= 3 then
                    FinanceOrganization.Office.Clear()
                end
                DrawOrganizationName('Lux', FinanceOrganization.Name.style, FinanceOrganization.Name.color, FinanceOrganization.Name.font)
                LastOfficeID = 3
                FinanceOrganization.Office.loaded = true
                Wait(0) -- We need to call all this every frame
            elseif (Global.FinanceOffices.isInsideOffice1) then
                if LastOfficeID ~= 1 then
                    FinanceOrganization.Office.Clear()
                end
                DrawOrganizationName('Fernandez A/S', FinanceOrganization.Name.style, FinanceOrganization.Name.color, FinanceOrganization.Name.font)
                LastOfficeID = 1
                FinanceOrganization.Office.loaded = true
                Wait(0) -- We need to call all this every frame
            elseif (Global.FinanceOffices.isInsideOffice4) then
                if LastOfficeID ~= 4 then
                    FinanceOrganization.Office.Clear()
                end
                DrawOrganizationName('AF koncern', FinanceOrganization.Name.style, FinanceOrganization.Name.color, FinanceOrganization.Name.font)
                LastOfficeID = 4
                FinanceOrganization.Office.loaded = true
                Wait(0) -- We need to call all this every frame
            else
