			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						TriggerServerEvent('esx_society:removeVehicleFromGarage', 'exotic', vehicleProps)
				...
				local elements = {
					...
					{label = _U('caracara'),  value = 'aecara'}
				}
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 339.0, function(vehicle)
							...
							exports['t1ger_keys']:SetVehicleLocked(vehicle, 1)
								exports['t1ger_keys']:GiveTemporaryKeys(plate, "Exotic Bil", "Mekaniker")
						end)
					else
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_exotic', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
					...
					if NPCOnJob then
						if NPCTargetDeleterZone then
							...
							if CurrentlyTowedVehicle == NPCTargetTowable then
								...
								TriggerServerEvent('esx_mechanicjob:onNPCJobMissionCompleted', securityToken)
								StopNPCJob()
...
AddEventHandler('esx_mechanicjob:hasEnteredMarker', function(zone)
	...
	elseif zone == 'VehicleDeleter' then
		local playerPed = PlayerPedId()
		if IsPedInAnyVehicle(playerPed, false) and ESX.PlayerData.job.name == 'exotic' then
			local vehicle = GetVehiclePedIsIn(playerPed,  false)
...
AddEventHandler('esx_mechanicjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'exotic' and not IsPedInAnyVehicle(playerPed, false) then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'exotic' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'exotic', vehicleProps)
						...
						then
							TriggerServerEvent('esx_service:disableService', 'exotic')
						end
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'exotic' then
			OpenMobileMechanicActionsMenu()
		end

		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'exotic' then
