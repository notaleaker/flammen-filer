...
RegisterNetEvent("esx:reward")
AddEventHandler("esx:reward", function(src)
	jailTime = jailTime - 5
...
RegisterNetEvent("esx2-qalle-jail:jailPlayer")
AddEventHandler("esx2-qalle-jail:jailPlayer", function(newJailTime)
	jailTime = newJailTime
...
function JailLogin()
	...
	ESX.ShowNotification("Sidste gang du loggede ud var du i fængsel, derfor er du her igen!")
...
function UnJail()
	...
	ESX.ShowNotification("Du er blevet løsladt!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Du har " .. jailTime .. " minutter tilbage i fængsel!")
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'top-left',
			elements = {
				{ label = "Smid nærmeste person i fængsel", value = "jail_closest_player" },
				{ label = "Løslad person", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Tiden skal være i minutter!")
            	else
              		menu2.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Fængslet er tomt!")
					return
				end
				-- print(json.encode(playerArray))
				print(playerArray.status)
				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Person: " .. playerArray[i].name .. " | Fængsels tid: " .. playerArray[i].jailTime .. " minutter" .. " | Status ".. playerArray[i].status, value = playerArray[i].identifier })
				end
