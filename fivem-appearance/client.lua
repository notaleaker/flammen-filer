...
RegisterCommand('skinped', function()
	local config = {
		ped = true,
		headBlend = false,
		faceFeatures = false,
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInClothingShop and not hasAlreadyEnteredMarker) or (isInClothingShop and LastZone ~= currentZone) then
			...
			TriggerEvent('cd_drawtextui:ShowUI', 'show', "Tryk [E] For at ændre tøj")
		end
		...
		if (isInBarberShop and not hasAlreadyEnteredMarker) or (isInBarberShop and LastZone ~= currentZone) then
			...
			TriggerEvent('cd_drawtextui:ShowUI', 'show', "Tryk [E] for at ændre hår/skæg")
		end
...
RegisterNetEvent('fivem-appearance:pickHouseNewOutfit', function(data)
    local id = data.id
