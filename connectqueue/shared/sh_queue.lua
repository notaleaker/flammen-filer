...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] blev prioteret før dig, og endte %d/%d i køen", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] blev placeret som nummer %d/%d i køen", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        Queue:DebugPrint("En spiller kunne ikke connecte.")
    end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if not ids then
        ...
        CancelEvent()
        Queue:DebugPrint("Dropped " .. name .. ", kunne ikke hente information om steam ID")
        return
    end
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Du er blevet blokkeret for at joine")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] blev blokkeret for at joine; Grund: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] rejoinede køen efter at have forladt den", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] loader ind på serveren")
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("[Kø] Fjernet fra kø, data ugyldig :(")
            Queue:DebugPrint(tostring(name .. "[" .. ids[1] .. "] blev fjernet fra køen, fordi de havde ugyldigt data"))
            return
        end
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("[Kø] Fjernet grundet timeout")
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] blev fjernet fra køen, grundet timeout")
            else
                i = i + 1
            end
        end
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [Kø] Disabling hardcap ^7")
...
commands.setpos = function(args)
    ...
    Queue:DebugPrint("SET " .. data.name .. "'s KØ POSITION TIL: " .. args[2])
end
