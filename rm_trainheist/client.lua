    ...
    ESX.TriggerServerCallback('trainheist:server:checkPoliceCount', function(status)
        if status then
            ESX.TriggerServerCallback('trainheist:server:checkTime', function(time)
                ...
                --   TriggerServerEvent("mdt:newCall", "Butiksrøveri igang!", "Ukendt", vector3(playerCoords.x, playerCoords.y, playerCoords.z))
                --	TriggerServerEvent("esx_phone:send", "police", "GPSيـوجد سـطو على بقالة تم رصد المكان عن طريق ال!", true, { ["x"] = x, ["y"] = y, ["z"] = z })
                TriggerServerEvent('esx_outlawalert:trainrobberyInProgress', {
                    x = ESX.Math.Round(playerCoords.x, 1),
                    y = ESX.Math.Round(playerCoords.y, 1),
                    z = ESX.Math.Round(playerCoords.z, 1)
                }, streetName, playerGender, playerCoords)
                end
            end)
        end
    end)
...
function SpawnGuards()
    ...
    for k, v in pairs(Config['TrainHeist']['guardPeds']) do
        ...
        SetPedArmour(TrainHeist['guardPeds'][k], 200)
