...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(10000)
    ESX.TriggerServerCallback('esx_lscustomdriftisland:getVehiclesPrices', function(vehicles)
        Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_lscustomdriftisland:cancelInstallMod')
AddEventHandler('esx_lscustomdriftisland:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	ESX.Game.SetVehicleProperties(vehicle, myCar)
end)

RegisterNetEvent('esx_lscustomdriftisland:repair')
AddEventHandler('esx_lscustomdriftisland:repair', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	SetVehicleFixed(vehicle)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	startCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_lscustomdriftisland:setStarterCarServer', startCar)
	mainPrice = {}
	totalPrice = 0
	GetAction({value = 'main'})
end)

RegisterNetEvent('esx_lscustomdriftisland:GoOutOfLS')
AddEventHandler('esx_lscustomdriftisland:GoOutOfLS', function()
...
RegisterNetEvent('esx_lscustomdriftisland:setVehicleProps')
AddEventHandler('esx_lscustomdriftisland:setVehicleProps', function(props)
...
function OpenPayMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pay_menu', {
		...
		if data.current.value == 'no' then
			...
			myCar = {}
			startCar = {}
			TriggerServerEvent('esx_lscustomdriftisland:removeStarterCarServer')
		elseif data.current.value == 'yeas' then
			ESX.TriggerServerCallback('esx_lscustomdriftisland:hasMoneyToExit', function(paid)
				if paid then
					menu.close()
					lsMenuIsShowed = false
					local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
					FreezeEntityPosition(vehicle, false)
					TriggerServerEvent('esx_lscustomdriftisland:refreshOwnedVehicle', myCar)
					TriggerServerEvent('esx_lscustomdriftisland:removeStarterCarServer')
					myCar = {}
					startCar = {}
				end
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				if (data.current.label == " ".._U('by_default') and GetVehicleMod(vehicle, v.modType) ~= -1) or string.match(data.current.label, _U('installed')) then
					...
					if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'driftisland' then
		...
		menu.close()
		TriggerEvent('esx_lscustomdriftisland:cancelInstallMod')
...
function OpenRepairMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ls_repair', {
		...
		if data.current.value == 'repair' then
			menu.close()
			TriggerServerEvent("esx_lscustomdriftisland:buyrepair",  price)
		end
	end, function(data, menu)
		menu.close()
		local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local lastZone    = nil
			local vehicle = GetVehiclePedIsIn(playerPed, false)
			if (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'driftisland') or Config.IsMechanicJobOnly == false then
				for k,v in pairs(Config.Zones) do
			...
			if IsControlJustReleased(0, Keys['E']) and not lsMenuIsShowed and isInLSMarker then
				if not IsSpecial or (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'driftisland') then
