...
function AddEmoteMenu(menu)
    ...
    local jobmenu = _menuPool:AddSubMenu(submenu, Config.Languages[lang]['jobemotes'], "", "", Menuthing, Menuthing)
    ...
    for a,b in pairsByKeys(DP.JobEmotes) do
      x,y,z = table.unpack(b)
      jobitem = NativeUI.CreateItem(z, "/e ("..a..")")
      jobmenu:AddItem(jobitem)
      table.insert(JobTable, a)
      if not Config.SqlKeybinding then
        jobfavitem = NativeUI.CreateItem(z, Config.Languages[lang]['set']..z..Config.Languages[lang]['setboundemote'])
        favmenu:AddItem(jobfavitem)
        table.insert(FavEmoteTable, a)
      end
    end
    ...
    jobmenu.OnItemSelect = function(sender, item, index)
      EmoteMenuStart(JobTable[index], "jobs")
    end
