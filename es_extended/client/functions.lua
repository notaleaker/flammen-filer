...
ESX.Game.GetVehicleProperties = function(vehicle)
	if DoesEntityExist(vehicle) then
		...
		local color3, color4 = nil, nil

		if GetIsVehiclePrimaryColourCustom(vehicle) == 1 then
			local primCustomColor_R, primCustomColor_G, primCustomColor_B = GetVehicleCustomPrimaryColour(vehicle)
			color3 = {}
			color3[1] = primCustomColor_R
			color3[2] = primCustomColor_G
			color3[3] = primCustomColor_B
		end

		if GetIsVehicleSecondaryColourCustom(vehicle) == 1 then
			local secondCustomColor_R, secondCustomColor_G, secondCustomColor_B = GetVehicleCustomSecondaryColour(vehicle)
			color4 = {}
			color4[1] = secondCustomColor_R
			color4[2] = secondCustomColor_G
			color4[3] = secondCustomColor_B
		end
		...
		local extras = {}
		damages = {['damaged_windows'] = {}, ['burst_tires'] = {}, ['broken_doors'] = {}}
		for extraId=0, 12 do
		...
		return {
			...
			modHeadlight      = GetVehicleXenonLightsColour(vehicle),
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	if DoesEntityExist(vehicle) then
		...
		if props.modHeadlight then SetVehicleXenonLightsColour(vehicle, props.modHeadlight) end
		...
		if props.damages ~= nil then
			for i = 0, 13 do
				if props.damages['damaged_windows'] then
					if props.damages['damaged_windows'][i] then
						SmashVehicleWindow(vehicle, props.damages['damaged_windows'][i])
					end
				end
				if props.damages['burst_tires'] then
					if props.damages['burst_tires'][i] then
						SetVehicleTyreBurst(vehicle, props.damages['burst_tires'][i], true, 1000.0)
					end
				end
				if props.damages['broken_doors'] then
					if props.damages['broken_doors'][i] then
						SetVehicleDoorBroken(vehicle, props.damages['broken_doors'][i], true)
					end
				end
			end
		end
