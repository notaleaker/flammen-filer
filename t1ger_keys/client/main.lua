...
function ToggleVehicleLock()
	...
	if DoesEntityExist(vehicle) then
		...
		exports['mythic_notify']:DoHudText('inform', 'Der ingen køretøjer i nærheden')
	end
end
...
function UpdateVehicleLocked(vehicle)
	...
	if DecorGetInt(vehicle, lock_decor) == 1 or DecorGetInt(vehicle, lock_decor) == 0 then
		...
		exports['mythic_notify']:DoHudText('error', 'Du låste køretøjet')
	elseif DecorGetInt(vehicle, lock_decor) == 2 or DecorGetInt(vehicle, lock_decor) == 10 then
		...
		exports['mythic_notify']:DoHudText('success', 'Du låste op for køretøjet')
	end
...
function HasJobVehicleKey(plate)
	if job_keys ~= nil and job_keys[plate] ~= nil then
...
function ToggleVehicleEngine()
	...
	if DecorGetBool(vehicle, engine_decor) then
		...
		exports['mythic_notify']:DoHudText('error', 'Du slukket for køretøjet')
	else
		DecorSetBool(vehicle, engine_decor, true)
		exports['mythic_notify']:DoHudText('success', 'Du tændte for køretøjet')
	end
	...
	ESX.TriggerServerCallback('t1ger_keys:fetchOwnedVehicles', function(results) 
		if next(results) then
			...
			if next(elements) == nil then
				...
				function(data, menu)
					--menu.close()
					local fetched, model, price = false, nil, 100 -- 0
					...
					if price <= 0 then
						...
						price = 5500--math.floor((val.price/100) * price)
					end
