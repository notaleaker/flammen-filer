...
RegisterCommand('kørestol', function(source)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		...
		exports['mythic_notify']:DoLongHudText('error', 'Du har ikke adgang til kørestole')
		-- TriggerServerEvent('qalle:notify',source)
	end
end, false)
...
Sit = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'missfinale_c2leadinoutfin_c_int', '_leadin_loop2_lester', 3) then
			ShowNotification("Nogle bruger allerede denne kørestol!")
			return
		end
	end
...
PickUp = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'anim@heists@box_carry@', 'idle', 3) then
			ShowNotification("Nogle køre allerede kørestolen!")
			return
		end
	end
