...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName('DLA Revisor')
...
Citizen.CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'dla' then
		...
		if data.current.value == 'test' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'fuck13di2nmor21', {
				...
				if test == nil then
					...
					menu.close()
					ESX.TriggerServerCallback("PP:MoneywashDLA", function(cb)
...
RegisterNetEvent("PP:TimerDLA")
AddEventHandler("PP:TimerDLA", function(timer)
...
AddEventHandler('frp_revisordla:hasEnteredMarker', function(zone)
...
AddEventHandler('frp_revisordla:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'dla' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('frp_revisordla:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('frp_revisordla:hasExitedMarker', LastZone)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'dla' then
